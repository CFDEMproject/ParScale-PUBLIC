...
%   USER INPUT
k=1;
data2PlotList{k} = 'speciesA.json';         k=k+1;
data2PlotList{k} = 'speciesSolid.json';     k=k+1;
...
gridPoint2Plot{j} = 6; symbol{j}='k<'; faceC{j}='k'; j=j+1;    %Middle
...
for iFile=1:size(allDirs,1)
    ...
    if(size(myFile,1)>0)
        ...
        i=particleID; %load current particle!
...
print('-depsc2',['plot_',data2Plot,'_',num2str(particleID),'.eps'])
